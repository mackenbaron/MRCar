...
CMFCPRDlg::CMFCPRDlg(CWnd* pParent /*=NULL*/)
	: CDialogEx(CMFCPRDlg::IDD, pParent)
{
	m_hIcon = AfxGetApp()->LoadIcon(IDR_MAINFRAME);
}

void CMFCPRDlg::DoDataExchange(CDataExchange* pDX)
{
	CDialogEx::DoDataExchange(pDX);
}

BEGIN_MESSAGE_MAP(CMFCPRDlg, CDialogEx)
	ON_WM_SYSCOMMAND()
	ON_WM_PAINT()
	ON_WM_QUERYDRAGICON()
	ON_BN_CLICKED(IDC_BUTTON_OPEN, &CMFCPRDlg::OnBnClickedButtonOpen)
END_MESSAGE_MAP()


// CMFCPRDlg 消息处理程序

BOOL CMFCPRDlg::OnInitDialog()
{
...
void CMFCPRDlg::OnSysCommand(UINT nID, LPARAM lParam)
{
...
void CMFCPRDlg::OnPaint()
{
	...
	{
		if (!m_img.empty())
			DrawMatToWnd(GetDlgItem(IDC_PIC), m_img);
		CDialogEx::OnPaint();
	}
}
...
//显示。
HCURSOR CMFCPRDlg::OnQueryDragIcon()
{
...
void CMFCPRDlg::OnBnClickedButtonOpen()
{
	// TODO:  在此添加控件通知处理程序代码
	//m_img = imread("test.jpg");
	CString strFile = _T("");
	CFileDialog    dlgFile(TRUE, NULL, NULL, OFN_HIDEREADONLY, _T("Image Files (*.jpg)|*.jpg|All Files (*.*)|*.*||"), NULL);
	if (dlgFile.DoModal())
	{
		strFile = dlgFile.GetPathName();
		m_img = imread((LPCSTR)CT2A(strFile.GetBuffer(0)));
		vector<string>licenses = RecognizeOneImage(m_img);
		CA2T szr(licenses[0].c_str());
		GetDlgItem(IDC_EDIT_RESULT)->SetWindowText((LPCTSTR)szr);
		Invalidate(FALSE);
	}
}
